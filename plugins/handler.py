...
def clear_user(client, hex, id_user, accept):
    if hex in bot.pending:
        if not accept:
            ...
            client.get_chat(bot.pending[hex]['chat_id']).restrict_member(id_user, ChatPermissions(
...
def callbackAnswer(client, callback_query):
    if 'accept' in callback_query.data:
        ...
        if hex in bot.pending and bot.pending[hex]['user_id'] == callback_query.from_user.id:
