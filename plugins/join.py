...
def timer_set(id_task: str, job, args: list, seconds=300):
    ...
    bot.scheduler.add_job(
        job, "date", run_date=timer, id=id_task, args=args, replace_existing=True
    )


@Client.on_message(
    (Filters.group & Filters.new_chat_members) | Filters.command("disclaimer"), group=-1
)
def handlerJoin(client, message):
    if (
        message.chat.id == -1001128902578 or message.chat.id == -1001244432522
    ):  # speck and memes or debug
            ...
            if not user.is_self:
                if not bot.admin(user.id):
                    try:
                        ...
                        msg = message.reply(
                            bot.settings["welcome"].format(
                                message.chat.title, user.first_name, user.id
                            )
                            if "welcome" in bot.settings
                            else "Welcome",
                        ...
                        timer_set(
                            f"{user.id}_{message.chat.id}",
                            ban_log,
                            args=[client, uuid_generated, user.id],
                        )
                        timer_set(
                            f"{message.message_id}_{message.chat.id}",
                            message.delete,
                            args=[],
                        )
                    except Exception as e:
                ...
                message.reply(
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                            [
                                InlineKeyboardButton(
                                    text="Accetto", callback_data=f"accept_{split[1]}"
                                )
                            ],
                            [
                                InlineKeyboardButton(
                                    text="Declino", callback_data=f"refuse_{split[1]}"
                                )
                            ],
                        ]
                    ),
