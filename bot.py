...
class Bot(metaclass=Singleton):
    ...
    def load(self) -> list:
        ...
        if "admins" not in self.settings or len(self.settings["admins"]) == 0:
            warn(
                "There's no admin for this bot. Set some admin to use this bot correctly."
            )
            self.settings["admins"] = []
        if "disclaimer" not in self.settings:
            warn(
                "There's no disclaimer for this bot. Set a disclaimer to use this bot correctly."
            )
            self.settings["disclaimer"] = "UNSET"
        return [
            self.settings["account_name"]
            if "account_name" in self.settings
            else "my_account",
